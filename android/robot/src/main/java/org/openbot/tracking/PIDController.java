...
public class PIDController {
    ...
    public Control update(TargetInfo target, Control lastControl) {
        ...
        double omegaServo = servoAngleCalculator.calculate(errorServoAngle, dt);
...
//        v_L = Math.max(-this.maxSpeed, Math.min(v_L, this.maxSpeed));
        ...
        if (abs(omegaServo) > (3.0 / 180.0 * PI / 0.1) * 1.5 * dt) {
            omegaServo = signum(omegaServo) * 3.0 / 180.0 * PI / 0.1 * 1.5 * dt;
        }
